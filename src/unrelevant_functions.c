...
void block_transpose(u32 *T, int length, int stride) {
        if(stride <= 32){
                ...
                if(stride&1) { // stride impair
                        for(int i = 1 ; i < stride ; i++){ // transpose la 1ere ligne et 1ere colonne
                                u32 tmp = T[i];
                                T[i] = T[i * length];
                                T[i * length] = tmp;
                        }
                        // Appel recursif avec le reste de la matrice carree
                        block_transpose(T + length + 1, length, --stride);
                }
                else { // stride pair

                        // Swap bloc 2 avec bloc 3
                        ...
                        block_transpose(block1, length, stride0); // bloc 1
                        block_transpose(block2, length, stride0); // bloc 2
                        block_transpose(block3, length, stride0); // bloc 3
                        block_transpose(block4, length, stride0); // bloc 4
                }
        }
}
...
void gemm_rec(long m, long n, long l, long strideU, long strideV, long strideW, u32 const *U, u32 const *V, u32 *W){
	...
	long l1 = l - l0;
        ...
        gemm_rec(m0, n0, l0, strideU, strideV, strideW, U, V, W); // AE
        gemm_rec(m1, n0, l0, strideU, strideV, strideW, U + m0 * strideU, V, W + m0 * strideW); // CE
        gemm_rec(m0, n0, l1, strideU, strideV, strideW, U, V + l0, W + l0); // AF
        gemm_rec(m1, n0, l1, strideU, strideV, strideW, U + m0 * strideU, V + l0, W + m0 * strideW + l0); // CF

        gemm_rec(m0, n1, l0, strideU, strideV, strideW, U + n0, V + n0 * strideV, W); // BG
        gemm_rec(m1, n1, l0, strideU, strideV, strideW, U + m0 * strideU + n0, V + n0 * strideV, W + m0 * strideW); // DG
        gemm_rec(m0, n1, l1, strideU, strideV, strideW, U + n0, V + n0 * strideV + l0, W + l0); // BH
        gemm_rec(m1, n1, l1, strideU, strideV, strideW, U + m0 * strideU + n0, V + n0 * strideV + l0, W + m0 * strideW + l0); // DH
}
