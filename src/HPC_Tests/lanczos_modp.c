...
MPI_Op mpi_sum_mod;
int size, rank;
int flag_orthogonalize, flag_block_dot_products, flag_sparse_matrix;
bool flag_csr;
long n = 1;
u64 prime;
...
#pragma omp declare reduction(sum_mod:u32:sum_mod(&omp_out, &omp_in)) \
...
void sparse_matrix_vector_product_csr_Original(u32 * y, struct sparsematrix_t const * M_csr, u32 const * x)
{
...
void sparse_matrix_vector_product_csr_OpenMP(u32 * y, struct sparsematrix_t const * M_csr, u32 const * x)
{
        int nrows = M_csr->nrows;
        int const * Mi = M_csr->i;
        int const * Mj = M_csr->j;
...
void sparse_matrix_vector_product_csr_MPI(u32 * y, struct sparsematrix_t const * M_csr, u32 const * x)
{
        int nrows = M_csr->nrows;
        int const * Mi = M_csr->i;
        int const * Mj = M_csr->j;
...
void sparse_matrix_vector_product_csr(u32 * y, struct sparsematrix_t const * M_csr, u32 const * x) {
        if (flag_sparse_matrix == 2)
                return sparse_matrix_vector_product_csr_OpenMP(y, M_csr, x);
        if (flag_sparse_matrix == 1)
                return sparse_matrix_vector_product_csr_MPI(y, M_csr, x);
        
        return sparse_matrix_vector_product_csr_Original(y, M_csr, x);

}
/* y += M*x or y += transpose(M)*x, according to the transpose flag */
void sparse_matrix_vector_product_Original(u32 * y, struct sparsematrix_t const * M, u32 const * x, bool transpose)
{
        long nnz = M->nnz;
...
void sparse_matrix_vector_product_OpenMP(u32 * y, struct sparsematrix_t const * M, u32 const * x, bool transpose)
{
        long nnz = M->nnz;
        int nrows = transpose ? M->ncols : M->nrows;
        int const * Mi = transpose ? M->j : M->i;
...
void sparse_matrix_vector_product_MPI(u32 * y, struct sparsematrix_t const * M, u32 const * x, bool transpose)
{
        long nnz = M->nnz;
        int nrows = transpose ? M->ncols : M->nrows;
        int const * Mi = transpose ? M->j : M->i;
...
{
        if(flag_sparse_matrix == 2)
                return sparse_matrix_vector_product_OpenMP(y, M, x, transpose);
        if(flag_sparse_matrix == 1)
                return sparse_matrix_vector_product_MPI(y, M, x,transpose);

        return sparse_matrix_vector_product_Original(y, M, x, transpose);
}
...
void block_dot_products_OpenMP(u32 * vtAv, u32 * vtAAv, int N, u32 const * Av, u32 const * v)
{
...
void block_dot_products_MPI(u32 * vtAv, u32 * vtAAv, int N, u32 const * Av, u32 const * v)
{
        memset(vtAAv, 0, n * n * sizeof(u32));
        memset(vtAv,  0, n * n * sizeof(u32));
...
void block_dot_products_Original(u32 * vtAv, u32 * vtAAv, int N, u32 const * Av, u32 const * v)
{
        memset(vtAv,  0, n * n * sizeof(u32));
        memset(vtAAv, 0, n * n * sizeof(u32));
...
{
        if(flag_block_dot_products == 2)
                return block_dot_products_OpenMP(vtAv, vtAAv, N, Av, v);
        if(flag_block_dot_products == 1)
                return block_dot_products_MPI(vtAv, vtAAv, N, Av, v);

        return block_dot_products_Original(vtAv, vtAAv, N, Av, v);
}
/* Compute the next values of v (in tmp) and p */
void orthogonalize_OpenMP(u32 * v, u32 * tmp, u32 * p, u32 * d, u32 const * vtAv, const u32 *vtAAv, 
...
void orthogonalize_MPI(u32 * v, u32 * tmp, u32 * p, u32 * d, u32 const * vtAv, const u32 *vtAAv, 
        u32 const * winv, int N, u32 const * Av)
{
        /* compute the n x n matrix c */
        u32 spliced[n * n];
...
void orthogonalize_Original(u32 * v, u32 * tmp, u32 * p, u32 * d, u32 const * vtAv, const u32 *vtAAv, 
        u32 const * winv, int N, u32 const * Av)
{
        /* compute the n x n matrix c */
        u32 spliced[n * n];
...
void orthogonalize(u32 * v, u32 * tmp, u32 * p, u32 * d, u32 const * vtAv, const u32 *vtAAv, 
        u32 const * winv, int N, u32 const * Av)
{
        if(flag_orthogonalize == 2)
                return orthogonalize_OpenMP(v, tmp, p, d, vtAv, vtAAv, winv, N, Av);
        if(flag_orthogonalize == 1)
                return orthogonalize_MPI(v, tmp, p, d, vtAv, vtAAv, winv, N, Av);

        return orthogonalize_Original(v, tmp, p, d, vtAv, vtAAv, winv, N, Av);
}
...
u32 * block_lanczos_OpenMP(struct sparsematrix_t const * M, int n, bool transpose, bool csr)
{
        ...
        while (true) {
                ...
                if(++cpt == (expected_iterations/10)){ // Save every 10%
...
u32 * block_lanczos_MPI(struct sparsematrix_t const * M, int n, bool transpose, bool csr)
{
        if (rank == 0)
        printf("Block Lanczos\n");
...
int main_MPI(int argc, char ** argv)
{
        ...
        send_coo(&M);
        u32 *kernel = block_lanczos_MPI(&M, n, right_kernel, flag_csr);
...
int main_OpenMP(int argc, char ** argv)
{
        ...
        u32 *kernel = block_lanczos_OpenMP(&M, n, right_kernel, flag_csr);
        if (kernel_filename)
                save_vector_block(kernel_filename, right_kernel ? M.ncols : M.nrows, n, kernel);
        else
                printf("Not saving result (no --output given)\n");
...
{
        // 0 = original, 1 = MPI, 2 = OpenMP
        flag_orthogonalize      = 2;
        flag_block_dot_products = 2;
        flag_sparse_matrix      = 2;
        flag_csr                = true;

        if (flag_orthogonalize != 1 && flag_block_dot_products != 1 && flag_sparse_matrix != 1)
                return main_OpenMP(argc, argv);
        else
